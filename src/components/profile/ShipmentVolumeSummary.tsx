        ...
        <div className="flex flex-col items-start relative flex-1 grow w-full">
          <div className="flex flex-col items-start gap-4 relative self-stretch w-full flex-[0_0_auto]">
            <div className="flex items-center gap-1 relative self-stretch w-full flex-[0_0_auto]">
              <div className="w-fit [font-family:'Satoshi-Bold',Helvetica] font-bold text-base sm:text-xl leading-[30px] relative mt-[-1.00px] text-text dark:text-textDark tracking-[0]">
                ...
                {TABS.map((tab) => (
                  <button
                    ...
                    className={`p-3 sm:px-4 sm:py-2 whitespace-nowrap text-sm sm:text-base font-medium transition-all border-b-2 ${activeTab === tab
                      ? "text-primary border-b-2 border-primary dark:border-primarySecondary font-bold"
                      : "text-textSecondary dark:text-textSecondaryDark border-b-2 border-border dark:border-borderDark"
                      }`}
                  >
                    {tab}
                  </button>
            ...
            <div className="w-full p-3 sm:px-4 sm:py-6">
              {activeTab === "Lanes used" && (
                <div className="flex flex-col items-start gap-2 relative self-stretch w-full flex-[0_0_auto]">
                  <div className="flex flex-col items-start gap-6 relative self-stretch w-full flex-[0_0_auto] rounded-2xl">
                    <div className="flex flex-col gap-6 w-full overflow-x-auto">
                      {lanesUsed.map((item => (
                        <div key={item.id} className="w-[500px] sm:w-full flex justify-between [font-family:'Satoshi',Helvetica] font-medium text-sm sm:text-base dark:text-textDark">
              ...
              {activeTab === "Containers used" && (
                <div className="flex flex-col gap-6">
                  {containersused.map((item => (
                    <div key={item.id} className="flex justify-between gap-2 [font-family:'Satoshi',Helvetica] font-medium text-sm sm:text-base dark:text-textDark">
                      <span>{item.title}</span>
                      <span>{item.value}</span>
                    </div>
                  )))}
            ...
            <div className="flex flex-col lg:min-[1025px]:flex-row items-center justify-between gap-6 sm:gap-0 w-full">
              ...
              <div className="flex flex-col items-center gap-4 sm:gap-8 flex-1">
                <div className="text-text dark:text-textDark font-bold text-base sm:text-xl text-center leading-8 sm:min-h-[64px] flex items-start justify-center px-2">
                ...
                <div className="flex flex-col gap-6 w-full items-center">
                  ...
                  <div className="relative w-[200px] h-[140px]">
                    ...
                    {/* Percentage Labels */}
                    <div className={`absolute left-[-30px] top-[30px] text-sm font-semibold ${isFCLBigger ? 'text-primary' : 'text-text dark:text-textDark'
                      }`}>
                      {fclPercent}%
                    </div>
                    <div className={`absolute right-[-35px] bottom-[25px] text-black dark:text-white text-sm font-semibold ${!isFCLBigger ? 'text-primary' : 'text-text dark:text-textDark'
                      }`}>
                      {lclPercent}%
                    </div>
                  </div>
                  <div className="flex flex-col gap-1.5 text-sm text-text dark:text-textDark">
                    {fclLclData.map((item) => (
              ...
              <div className="flex flex-col items-center gap-4 sm:gap-8 flex-1">
                ...
                <div className="flex flex-col gap-6 w-full items-center">
                  ...
                  <div className="relative w-[200px] h-[140px]">
                    ...
                    <div className={`absolute left-[-30px] top-[30px] text-sm font-semibold ${isHouseBigger ? 'text-primary' : 'text-text dark:text-textDark'
                      }`}>
                      {housePercent}%
                    </div>
                    <div className={`absolute right-[-35px] bottom-[25px] text-sm font-semibold ${!isHouseBigger ? 'text-primary' : 'text-text dark:text-textDark'
                      }`}>
                      {regularPercent}%
                    </div>
                  </div>
                  <div className="flex flex-col gap-2 text-sm text-text dark:text-textDark">
              ...
              <div className="flex flex-col items-center gap-4 sm:gap-8 flex-1">
                ...
                <div className="flex flex-col gap-6 w-full items-center">
                  ...
                  <div className="relative w-[200px] h-[140px]">
                    ...
                    <div className={`absolute left-[-30px] top-[30px] text-sm font-semibold ${isWithNotifyBigger ? 'text-primary' : 'text-text dark:text-textDark'
                      }`}>
                      {withNotifyPercent}%
                    </div>
                    <div className={`absolute right-[-35px] bottom-[25px] text-sm font-semibold ${!isWithNotifyBigger ? 'text-primary' : 'text-text dark:text-textDark'
                      }`}>
