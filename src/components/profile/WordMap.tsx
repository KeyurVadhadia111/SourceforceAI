...
const MAJOR_PORTS: Port[] = [
    // Asia Pacific
    { id: 'hulutao', name: 'Hulutao', country: 'China', lat: 40.712, lng: 120.836, isHub: false, region: 'Asia Pacific' },
    ...
    { id: 'laemchabang', name: 'Laem Chabang', country: 'Thailand', lat: 13.086, lng: 100.900, isHub: false, region: 'Asia Pacific' },
    { id: 'singapore', name: 'Singapore', country: 'Singapore', lat: 1.3521, lng: 103.8198, isHub: true, region: 'Asia Pacific' },

    // North America
    { id: 'tacoma', name: 'Tacoma, Wa', country: 'USA', lat: 47.2529, lng: -122.4443, isHub: false, region: 'North America' },
    { id: 'newyork', name: 'New York/Newark', country: 'USA', lat: 40.7128, lng: -74.0060, isHub: true, region: 'North America' },
    { id: 'oakland', name: 'Oakland, Ca', country: 'USA', lat: 37.8044, lng: -122.2711, isHub: false, region: 'North America' },
    { id: 'losangeles', name: 'Los Angeles, Ca', country: 'USA', lat: 34.0522, lng: -118.2437, isHub: true, region: 'North America' },
    { id: 'savannah', name: 'Savannah, Ga', country: 'USA', lat: 32.0809, lng: -81.0912, isHub: false, region: 'North America' },
    { id: 'porteverglades', name: 'Port Everglades, Fl', country: 'USA', lat: 26.0899, lng: -80.1592, isHub: false, region: 'North America' },

    // South America
    { id: 'bolboa', name: 'Bolboa', country: 'Panama', lat: 8.9526, lng: -79.559, isHub: false, region: 'South America' },
    { id: 'sanantonio', name: 'San Antonio', country: 'Chile', lat: -33.5936, lng: -71.6127, isHub: false, region: 'South America' },

    // Europe
    { id: 'hamble', name: 'Hamble', country: 'UK', lat: 50.8605, lng: -1.3169, isHub: false, region: 'Europe' },
    { id: 'stadersand', name: 'Stadersand', country: 'Germany', lat: 53.6167, lng: 9.4667, isHub: false, region: 'Europe' },
...
const PORT_CONNECTIONS = [
    ...
    { from: 'rotterdam', to: 'newyork' },
    { from: 'losangeles', to: 'oakland' },
    { from: 'sanantonio', to: 'bolboa' },
    { from: 'newyork', to: 'oakland' },
    { from: 'newyork', to: 'tacoma' },
    { from: 'newyork', to: 'savannah' },
    { from: 'newyork', to: 'porteverglades' }
];

const REGION_COLORS: Record<string, string> = {
    "Asia Pacific": "#fde68a",
    "North America": "#a5f3fc",
    "South America": "#bbf7d0",
    "Europe": "#c7d2fe",
    "Africa": "#fecaca",
};

const REGION_COUNTRIES: Record<string, string[]> = {
    "Asia Pacific": ["India", "China", "Japan", "Australia", "South Korea", "Indonesia"],
...
const REGION_BORDER_COLORS: Record<string, string> = {
    "Asia Pacific": "#facc15",      // yellow-400
    "North America": "#22d3ee",     // cyan-400
    "South America": "#4ade80",     // green-400
    "Europe": "#818cf8",            // indigo-400
...
const generateSampleData = (): ShipmentData[] => {
    ...
    const countries = [
        'China', 'Japan', 'Thailand', 'Singapore', 'USA',
        'Panama', 'Chile', 'UK', 'Germany', 'Netherlands'
    ];
...
const WorldMap: React.FC<WorldMapProps> = ({
    ...
    const countryData = useMemo(() => {
        ...
        const baseData = filteredData.reduce((acc, item) => {
            if (!acc[item.country]) {
                acc[item.country] = {
                    volume: 0,
                    weight: 0,
        ...
        // 🔁 Add missing countries (based on selected ports) with 0 data
        filteredPorts.forEach(port => {
            if (!baseData[port.country]) {
                baseData[port.country] = {
                    volume: 0,
                    weight: 0,
                    teu: 0,
                    seaShipment: 0,
                    ports: MAJOR_PORTS.filter(p => p.country === port.country).length
                };
            }
        });

        return baseData;
    }, [data, filters.fromYear, filters.toYear, filteredPorts]);


    const filteredConnections = useMemo(() => {
        return PORT_CONNECTIONS.filter(conn => {
            const fromPort = filteredPorts.find(p => p.id === conn.from);
            const toPort = filteredPorts.find(p => p.id === conn.to);
...
Country TEU: ${formatNumber(data.teu)} containers`;
    ...
    const renderWorldMap = () => {
        return (
            <div className="relative w-full mt-5 sm:mt-0 sm:h-[400px] rounded-lg overflow-hidden">
                <ComposableMap
                                        ...
                                        <Geography
                                            ...
                                            style={{
                                                default: {
                                                    ...
                                                    stroke: getRegionStrokeColor(countryName),
                                                    strokeWidth: 0.5,
                                                    outline: 'none',
                                                },
                                                hover: {
    ...
    return (
        <div className={`relative rounded-2xl ${className}`}>
            ...
            <div className="flex flex-col justify-center relative">
                <div className="box-border content-stretch flex flex-col items-start justify-center p-[16px] relative">
                    {/* Filters */}
                    {renderFilters()}
